...
import pgrad.sts.server.util.SOAPUtil;
...
//		node = xmldoc.createTextNode(MySTSConstants.TOKENTYPE_SAML11);
   ...
   private void setTokenTypeFromDOM(){
      ...
      if (e!=null) this.TokenType= e.getTextContent();
   }
	...
	private void setRequestTypeFromDOM() {
		...
		if (e != null) this.RequestType = e.getTextContent();
	}

	private void addAppliesTo(Document xmldoc) {
		
		if(this.AppliesToAddress==null) return;
	...
	private void setAppliesToFromDOM() {
		...
		if (nl.getLength()==0) return;
		if (e != null) this.AppliesToAddress = nl.item(0).getTextContent();
	}
	
	private void addIssuer(Document xmldoc) {
		
		//if(this.PolicyName==null) return;
		if(this.IssuerAddress==null) return;
	...
	*/
	private void setIssuerAddressFromDOM() {
		...
		NodeList nl=e.getElementsByTagName(MySTSConstants.WSA_PREFIX+":Address");
		//if (nl.getLength()>0) this.PolicyName = nl.item(0).getTextContent();
		if (nl.getLength()>0) this.IssuerAddress = nl.item(0).getTextContent();
	}
	...
	private void addSecondaryParameters(Document xmldoc) {
		...
		if(this.SecondaryParametersRol==null) return;
		...
		if(this.SecondaryParametersUser==null) return;
	...
	private void setRoleFromDOM() {
		...
		if (nl.getLength()>0) this.SecondaryParametersRol = nl.item(0).getTextContent();
		nl=e.getElementsByTagName("User");
		if (nl.getLength()>0) this.SecondaryParametersUser = nl.item(0).getTextContent();
	}
